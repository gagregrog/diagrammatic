...
const formatCSVDataForNode = (column, option = null) => ({
  data: {
    column,
    option,
...
const getGroupOptions = (group, rows) => {
  ...
  return options.map((option) => formatCSVDataForNode(group, option));
};
...
class Tree extends Node {
  ...
  __buildTreeRecursively(leafKey, nodeKeys, rows) {
    const recurse = (node, rows, colName) => {
      ...
      if (groupOptions.length === 1) {
        ...
        node.append(...rows.map((row) => formatCSVDataForNode(leafKey, row[leafKey])));
  ...
  static async fromCSV(rawCsvData, parseOpts) {
    const { leafKey, nodeKeys, rows } = await processFile(rawCsvData, parseOpts);

    const tree = new Tree(formatCSVDataForNode(nodeKeys[0]));
    tree.__buildTreeRecursively(leafKey, nodeKeys, rows);
